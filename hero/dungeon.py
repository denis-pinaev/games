...
def killEnemy(dataj2, create, first=False):
    ...
    if dataj2.has_key("mission") and dataj2["mission"].has_key("entities"):
        ...
        if dataj2["mission"]["missionTargets"].has_key("units"):
...
def battleCreate():
    ...
    service = actionCommand
    method = 'battleCreate'
    dataString = '{"reason":"dungeon","debug":false,"data":null,"index":"default","ctr":%s,"sessionKey":"%s","method":"%s"}' % (getCTR(), sid, method)
