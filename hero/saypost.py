...
if len(sys.argv) > 1:
    ...
    raise Exception("#         corc0     natali1      polya2      nikita3      ulia4      vladimir5      yura6      lenaSv7     nagaina8   tanakan 9     VitalikSha10   vladimir 11    mari kramer 12    NAZAR 13    VanyaM 14     Oleg 15   Jenya16   LenaBRED17")
...
pids = ["124520", "29431585", "144536559", '218661879', '56518190', '217858589', '179499220', "169768611", "68487257", "160511757", "73940623", '93902559', '161702967', '74163736', '114233049', '11305565', '692795', '65706308']
...
def log(s, pr=False, new_file=False, file_name="hero_say_log_", use_data=True):
    if use_data: s = "[" + datetime.datetime.now().strftime('%d.%m %H:%M:%S')+"] " + s
    if pr: print s
    fatype = "hero_say_log_"+str(pid)
...
def battleSay():
    ...
    method = 'createPost'
    #dataString = '{"rnd":%s,"index":"default"}' % (getRandom())
    #{"method":"createPost","image":"default","userLocale":"russian","imgTemplate":"","storyAction":"beat","cdn":"","storyId":"17691114","locKey":"#OG_PVP_WIN_DESC","text":"В нелёгком бою, мной одержана мастерская победа над войсками другого игрока! За победу была получена хорошая награда!","ctr":5,"storyType":"profile","sessionKey":"540c2d289f8172.82359643"}
...
person = 9
init_person()
for i in range(phaza):
    o = battleSay()
    log(sss+o['hash'], True, True, "hero_post", False)
